...
def constant_curvature(theta, phi, L):
    ...
    kappa = theta / L
    x_plane = (1 / kappa) * (1 - np.cos(theta))
    z = (1 / kappa) * np.sin(theta)
    
    x = x_plane * np.cos(phi)
    y = x_plane * np.sin(phi)
...
class SoftArmSection:
    ...
    def solve_forward_kinematics(self, l_known_vec: np.ndarray, 
            verbose: bool = False) -> Union[Tuple[float, float], Tuple[None, None]]:
        ...
        # when the first attempt fails, try the initial value starting from zero
        X0_simple = np.zeros(2 + 3 * self.n)
        X0_simple[0] = 0.01  
        X0_simple[2:2+self.n] = 1.0  
        X0_simple[2+2*self.n:] = 0.01 
        
        solution2, infodict2, ier2, mesg2 = fsolve(
            self._forward_kinematics_residuals, 
            X0_simple, 
            args=(l_known_vec,),
            full_output=True,
        ...
        if ier2 == 1:
            return solution2[0], solution2[1]
        
        # when the second attempt fails, check if the residual is small enough (even if fsolve reports failure)
        residual_norm = np.linalg.norm(infodict['fvec'])
        if residual_norm < 1e-3:
            return solution[0], solution[1]
        
        # when the third attempt fails, use the geometric approximation method
        kappa_approx, gamma_approx = self.approximate_forward_kinematics(l_known_vec)
        if kappa_approx is not None:
            return kappa_approx, gamma_approx
    ...
    def approximate_forward_kinematics(self, l_known_vec: np.ndarray) -> Tuple[float, float]:
        """
        使用几何近似方法计算正向运动学（当精确求解失败时使用）
        基于线性化假设：kappa_b ≈ (L - l_avg) / (L * d)
        """
        l_avg = np.mean(l_known_vec)
        delta_l = self.L - l_known_vec  
        
        # delta_l_i ≈ kappa_b * L * d * cos(alpha_i - gamma_g)
        A = np.zeros((self.n, 2))
        A[:, 0] = self.L * self.d * np.cos(self.cable_angles)
        A[:, 1] = self.L * self.d * np.sin(self.cable_angles)
        
        try:
            xy, _, _, _ = np.linalg.lstsq(A, delta_l, rcond=None)
            x, y = xy
            kappa_b = np.sqrt(x**2 + y**2)
            gamma_g = np.arctan2(y, x)
            
            kappa_b = np.clip(kappa_b, 0.0, 0.5)  
            
            return kappa_b, gamma_g
        except:
            return 0.0, 0.0
    ...
    def _forward_kinematics_residuals(self, X: np.ndarray, l_known_vec: np.ndarray) -> np.ndarray:
        ...
        residuals[2 + self.n : 2 + 2*self.n] = T_vec - (self.Kc / kappa_c_vec_safe) * (term1_vec - term2_vec)
